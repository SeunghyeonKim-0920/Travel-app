...
const checks = [
  ...
  ['room card actions preserve server room IDs', !files.app.includes("parseInt(e.currentTarget.getAttribute('data-room-id'), 10)") && files.app.includes("normalizeRoomId(e.currentTarget.getAttribute('data-room-id'))")],
  ['joined room ID is restored and saved canonically', files.app.includes('state.joinedRoomId = normalizeRoomId(localJoinedRoom);') && files.app.includes('state.joinedRoomId = canonicalRoomId;\n  saveToLocalStorage();')],
  ['remote room reads and writes share one queue', files.app.includes('let remoteSyncQueue = Promise.resolve();') && files.app.includes('.then(() => pullFromRemoteNow())') && files.app.includes('.then(() => pushToRemoteNow(options))')],
  ['legacy QA feedback is removed from every browser', files.app.includes("'mobile feedback verification'") && files.app.includes("'anonymous-feedback-qa'") && files.app.includes("'qfqgeeg'") && files.app.includes("safeSetLocalStorage('wander_feedbacks', JSON.stringify(state.feedbacks))")],
