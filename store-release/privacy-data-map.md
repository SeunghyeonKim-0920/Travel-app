...
| Safety reports, report reason, optional details, room/message evidence, installation IDs | Separate Cloudflare D1 moderation table | Receive and investigate companion safety reports | Report submission confirmation; contact support for access or deletion requests |
...
- Blocking is stored only on the current device; blocked participant content is filtered locally until unblocked.
- Moderation reports are write-only from the public client and are not included in the publicly readable shared application state.
...
- Confirm the moderation review owner, response-time target, escalation policy, and report-retention period before public store release.
