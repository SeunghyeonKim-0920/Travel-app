...
const checks = [
  ...
  ['localization audit covers accessibility text', /'data-i18n-aria-label'/.test(app)],
  ['localization audit rejects key literals and mojibake', /keyLiterals/.test(app) && /malformed/.test(app)],
  ['feedback source and translations are preserved', /translationSource:\s*text/.test(app) && /translations:\s*\{\s*\[language\]:\s*text\s*\}/.test(app)],
  ['feedback follows selected language', /getFeedbackDisplayText\(entry\)/.test(app) && /requestFeedbackTranslations\(entry, \[targetLanguage\]\)/.test(app)],
  ['feedback translation API is configured', /FEEDBACK_TRANSLATION_URL/.test(app) && /\/api\/translate/.test(app)],
  ['feedback dynamic copy uses translation keys', ['feedback_empty', 'feedback_required', 'feedback_rating_required', 'feedback_submitted', 'feedback_submit_failed'].every(key => app.includes(`getText('${key}')`) || app.includes(`getText(saved ? 'feedback_submitted' : 'feedback_submit_failed')`))],
