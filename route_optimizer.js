...
const TRAVEL_DB = {
  ...
  'frankfurt|interlaken': { train: { time: 315, note: '직통 ICE 약 5시간 15분 (DB/SBB 공식 노선 기준)', connectionType: 'direct' }, flight: null, bus: null },
...
const ROUTE_STAY_DAYS = Object.freeze({
  ...
  munich: [2, 3], vienna: [2, 4], prague: [2, 3], budapest: [2, 3], frankfurt: [2, 4], interlaken: [4, 7],
        ...
        if (seg.opt) {
          ...
          if (b.type === 'mixed') {
            ...
            if (hasBreakdown) {
              noteHTML = `
                <div class="segment-note" style="margin-top:6px; border-left: 2px solid var(--primary-color); padding-left: 10px;">
                  <strong>✈️ ${getRouteResultLabel('flightBreakdown', '출발지부터 도착지까지 항공 이동시간', 'Flight Duration Breakdown')}:</strong>
                  <ul style="margin:4px 0 0 16px; padding:0; list-style-type:disc; font-size:12px; color:var(--text-muted); line-height:1.4;">
...
function downloadRouteOffline() {
  ...
  let md = `# TripTogether ${isKo ? '다중 도시 최적 경로' : 'Multi-City Optimal Route'}\n\n`;
  ...
  md += `\n---\n*${isKo ? 'TripTogether AI 도시간 동선 짜기로 생성' : 'Generated by TripTogether AI Route Planner'}*`;
  try {
    const blob = new Blob([md], { type: 'text/plain;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `TripTogether_Route_${optimized.map(c => c.id).join('-')}_${Date.now()}.txt`;
